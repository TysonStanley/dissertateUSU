---
title: 'Dissertating for USU'
author: "You"
geometry: [top=1in, bottom=1in, right=1in, left=1.5in]
year: '2019'
degree: 'Degree'
field: 'Field'
department: "Your Department"
uni: 'UTAH STATE UNIVERSITY'
location: "Logan, UT"
chair: 'Chair Person'
committee2: 'Committee Member 1'
committee3: 'Committee Member 2'
committee4: 'Committee Member 3'
committee5: 'Committee Member 4'
gradschool: 'Richard S. Inouye, Ph.D.'
gradschoollabel: "Vice Provost for Graduate Studies"
bibliography: yourbibfile.bib
output:
  dissertateUSU::dissertateUSU:
    latex_engine: xelatex
    keep_tex: yes
    pandoc_args: [ "--csl", "apa6.csl" ]
---

```{r global_options, include=FALSE}
## This saves you from having to do this for every chunk
knitr::opts_chunk$set(fig.path = 'figures/',
                      echo = FALSE, 
                      warning = FALSE, 
                      message = FALSE)
```


<!-- Abstract -->
\newpage
\pagestyle{fancy}
\fancyhead[L]{Abstract}
\fancyhead[R]{\thepage}
\fancyfoot[C]{}
\chapter*{ABSTRACT}
\addcontentsline{toc}{section}{Abstract}

<!-- The stuff right below will autofill -->
\doublespacing
\begin{center}
`r rmarkdown::metadata$title`
\vspace{12pt}
by \\
\vspace{12pt}
`r rmarkdown::metadata$author` \\
`r rmarkdown::metadata$uni`, `r rmarkdown::metadata$year`
\end{center}
\vspace{12pt}
\singlespacing
\noindent Major Professor: `r rmarkdown::metadata$chair` \\
\noindent Department: `r rmarkdown::metadata$department`
\vspace{12pt}
<!-- end of autofill -->

\doublespacing

Your abstract words go here.


<!-- Public Abstract -->
\newpage
\fancyhead[L]{Public Abstract}
\fancyhead[R]{\thepage}
\fancyfoot[C]{}
\chapter*{PUBLIC ABSTRACT}
\addcontentsline{toc}{section}{Public Abstract}
\doublespacing

Your publically abstracted words go here.


<!-- Dedication -->
\newpage
\fancyhead[L]{Dedication}
\fancyhead[R]{\thepage}
\fancyfoot[C]{}
\chapter*{DEDICATION}
\addcontentsline{toc}{section}{Dedication}


Dedicate it.


<!-- Acknowledgements -->
\newpage
\fancyhead[L]{Acknowledgments}
\fancyhead[R]{\thepage}
\fancyfoot[C]{}
\chapter*{ACKNOWLEDGEMENTS}
\addcontentsline{toc}{section}{Acknowledgments}

Acknowledge those acknowledgable individuals and things.


<!-- Front Matter -->
\newpage
\fancyhead[L]{Table of Contents}
\fancyhead[R]{\thepage}
\fancyfoot[C]{}
\tableofcontents

\newpage
\fancyhead[L]{List of Tables}
\fancyhead[R]{\thepage}
\fancyfoot[C]{}
\listoftables

\newpage
\fancyhead[L]{List of Figures}
\fancyhead[R]{\thepage}
\fancyfoot[C]{}
\listoffigures


\newpage
\pagenumbering{arabic}

<!-- Chapter 1 -->
\newpage
\fancyhead[L]{Introduction}
\fancyhead[R]{\thepage}
\fancyfoot[C]{}

\chapter{INTRODUCTION}

```{r, child="Chapter1.Rmd"}
```


<!-- Chapter 2 -->
\FloatBarrier
\newpage
\fancyhead[L]{Chapter 2's Title}
\fancyhead[R]{\thepage}
\fancyfoot[C]{}

\chapter{Chapter 2's Title}

```{r, child="Chapter2.Rmd"}
```


<!-- Chapter 3 -->
\FloatBarrier
\newpage
\fancyhead[L]{Chapter 3's Title}
\fancyhead[R]{\thepage}
\fancyfoot[C]{}

\chapter{Chapter 3's Title}

```{r, child="Chapter3.Rmd"}
```


<!-- Chapter 4 -->
\FloatBarrier
\newpage
\fancyhead[L]{Chapter 4's Title}
\fancyhead[R]{\thepage}
\fancyfoot[C]{}

\chapter{Chapter 4's Title}

```{r, child="Chapter4.Rmd"}
```


<!-- Chapter 5 -->
\FloatBarrier
\newpage
\fancyhead[L]{Chapter 5's Title}
\fancyhead[R]{\thepage}
\fancyfoot[C]{}

\chapter{Chapter 5's Title}

```{r, child="Chapter5.Rmd"}
```



\FloatBarrier
\newpage
\fancyhead[L]{References}
\fancyhead[R]{\thepage}
\fancyfoot[C]{}

\chapter*{REFERENCES}

\setlength{\parindent}{-0.5in}
\setlength{\leftskip}{0.4in}
\setlength{\parskip}{6pt}
\noindent
