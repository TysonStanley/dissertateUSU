...
-  Currently, the package uses APA 6 formatting for citations (by default) but this can be altered by downloading a new `.csl` file and specifying it in the yaml, by replacing the `apa6.csl` in the `pandoc_args: [ "--csl", "apa6.csl" ]` argument with your file name.
